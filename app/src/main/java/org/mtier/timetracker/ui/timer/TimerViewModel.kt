...
data class DayGroup(val label: String, val items: List<WorkIntervalItemDto>)
...
data class EditNameDetailsState(val item: WorkIntervalItemDto, val name: String, val details: String)
data class EditTimeState(val item: WorkIntervalItemDto, val start: Instant, val end: Instant)
data class ManualEntryState(val name: String = "", val details: String = "", val start: Instant, val end: Instant)
...
class TimerViewModel @Inject constructor(
    ...
    fun refresh() {
        ...
        viewModelScope.launch {
            runCatching { timerRepository.getWorkIntervals(rangeFrom, rangeTo) }
                .onSuccess { response ->
                    val days = response.days.map { (label, nameGroups) ->
                        DayGroup(
                            label = label,
                            items = nameGroups.values
                                .flatMap { it.children }
    ...
    private fun startTimer(name: String, projectId: Int?, tagIds: List<String>) {
        viewModelScope.launch {
    ...
    fun updateProject(item: WorkIntervalItemDto, projectId: Int?) {
        viewModelScope.launch {
    ...
    fun updateTags(item: WorkIntervalItemDto, tagIdsOrNames: List<String>) {
        viewModelScope.launch {
    ...
    fun costDraft(item: WorkIntervalItemDto): String =
        costDrafts[item.id] ?: formatCents(item.cost ?: 0)

    fun onCostDraftChanged(item: WorkIntervalItemDto, value: String) {
