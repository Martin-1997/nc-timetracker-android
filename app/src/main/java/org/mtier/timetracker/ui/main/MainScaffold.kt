...
private fun iconFor(destination: Destination): ImageVector =
    when (destination) {
        ...
        Destination.Goals -> Icons.Filled.Flag
        Destination.Reports -> Icons.Filled.Assessment
        Destination.Timelines -> Icons.Filled.Timeline
        Destination.TimelinesAdmin -> Icons.Filled.AdminPanelSettings
...
private fun labelFor(destination: Destination): Int =
    when (destination) {
        ...
        Destination.Goals -> R.string.nav_goals
        Destination.Reports -> R.string.nav_reports
        Destination.Timelines -> R.string.nav_timelines
        Destination.TimelinesAdmin -> R.string.nav_timelines_admin
...
fun MainScaffold(
    ...
    // Timelines Admin is admin-only, same as the web frontend's
    // App.vue: `views.filter((item) => item.name !== 'timelines-admin' || this.isAdmin)`.
    val visibleDestinations = drawerDestinations.filter { it != Destination.TimelinesAdmin || isAdmin }
    ...
    ModalNavigationDrawer(
        ...
        if (showSignOutConfirm) {
            AlertDialog(
                ...
                dismissButton = {
                    Button(onClick = { showSignOutConfirm = false }) {
